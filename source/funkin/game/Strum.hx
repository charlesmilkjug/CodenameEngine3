		...
		if (shouldX || shouldY)
		{
			...
			else
			{
				...
				if (Std.int(daNote.__noteAngle % 360) != 0)
				{
					...
					var aOffset:FlxPoint = FlxPoint.get((daNote.origin.x / daNote.scale.x) - daNote.offset.x,
				...
				if (shouldX)
					daNote.x = x + realOffset.x;
				if (shouldY)
					daNote.y = y + realOffset.y;
